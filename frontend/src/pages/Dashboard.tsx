...
interface User { id: string; name: string; email: string; } // Interface para usuário
...
export const Dashboard: React.FC = () => {
  ...
  // NOVOS STATES PARA SELEÇÃO DE MEMBROS
  ...
  useEffect(() => {
      if (isCreating) {
          ...
          setSelectedMembers([]); // Limpa seleção ao fechar
      }
  }, [isCreating]);
  ...
  const handleCreateBoard = async (e: React.FormEvent) => {
    ...
    try {
      const res = await api.post('/boards', { 
          ...
          members: selectedMembers // Envia os membros
      });
  ...
  return (
    <div className="flex-1 p-8 md:p-12 overflow-y-auto bg-[#F8FAFC] dark:bg-[#0F1117] transition-colors h-full font-sans">
      <main className="max-w-7xl mx-auto">
        {/* ... (Header e Grid de Boards iguais ao anterior) ... */}
        {/* Vou pular para o MODAL para economizar espaço, mantenha o resto do código igual */}
        
        <div className="flex items-center gap-3 mb-10 opacity-90">
             {/* ... Header da página igual ... */}
             <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 animate-in fade-in duration-500">
            ...
            {boards.map(board => {
                ...
                // Pega os membros retornados pelo backend
                    ...
                    <div className="pl-2">
                        <div className="mb-3">
                            <span 
                                ...
                                style={{ color: accentColor, backgroundColor: theme === 'dark' ? `${accentColor}20` : `${accentColor}15` }}
                            >
                                Projeto
                            </span>
                        </div>
                        ...
                        <div className="flex -space-x-2 overflow-hidden">
                            {members.map((m: any, i: number) => (
                                <div 
                                    key={m.id || i}
                                    title={m.name}
                                    className="inline-flex h-7 w-7 rounded-full ring-2 ring-white dark:ring-[#1E2028] bg-gray-200 dark:bg-gray-600 items-center justify-center text-[9px] font-bold text-gray-600 dark:text-gray-300 uppercase"
                                >
                                    {m.name ? m.name.substring(0, 2) : '??'}
                                </div>
                            ))}
                            {/* Se houver muitos membros (o backend limita a 4), poderíamos mostrar um +N aqui */}
                        </div>
                    ...
                    <div>
                        ...
                        <div className="border border-gray-200 dark:border-gray-700 rounded-xl overflow-hidden max-h-40 overflow-y-auto custom-scrollbar bg-gray-50 dark:bg-[#0F1117]">
                            {isLoadingUsers ? (
                                ...
                                <div className="p-4 text-center text-xs text-gray-400">Nenhum outro usuário encontrado.</div>
                            ) : (
                                availableUsers.map(user => (
                                    <div 
                                        key={user.id} 
                                        onClick={() => toggleMember(user.id)}
                                        className={`flex items-center gap-3 p-3 cursor-pointer transition-colors border-b border-gray-100 dark:border-gray-800 last:border-0 ${selectedMembers.includes(user.id) ? 'bg-rose-50 dark:bg-rose-900/20' : 'hover:bg-gray-100 dark:hover:bg-[#1F222A]'}`}
                                    >
                                        <div className={`w-5 h-5 rounded border flex items-center justify-center transition-all ${selectedMembers.includes(user.id) ? 'bg-rose-500 border-rose-500' : 'border-gray-300 dark:border-gray-600'}`}>
                                            {selectedMembers.includes(user.id) && <svg className="w-3.5 h-3.5 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}><path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" /></svg>}
                                        </div>
                                        <div className="flex-1 min-w-0">
                                            <p className={`text-sm font-bold truncate ${selectedMembers.includes(user.id) ? 'text-rose-700 dark:text-rose-300' : 'text-gray-700 dark:text-gray-300'}`}>{user.name}</p>
                        ...
                        <p className="text-[10px] text-gray-400 mt-2 text-right">{selectedMembers.length} selecionado(s)</p>
                    </div>
                </div>
